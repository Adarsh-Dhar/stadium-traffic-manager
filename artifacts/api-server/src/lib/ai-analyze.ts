...
import type { SystemMetrics } from "./stadium-state.js";
...
function applyLocalHeuristic(metrics: SystemMetrics): { analysis: string; actions: string[]; confidence: number; serversAdded: number } {
  ...
  return { analysis, actions: defaultActions, confidence, serversAdded };
}
...
async function callModel(promptText: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // Authorization header removed — Gemini uses query param, not Bearer token
...
export async function aiAnalyze(): Promise<{
  ...
  try {
    ...
    if (parsed) {
      ...
      const fallback = applyLocalHeuristic(metrics);
      analysis = fallback.analysis;
      defaultActions.push(...fallback.actions);
      confidence = fallback.confidence;
      serversAdded = fallback.serversAdded;
    }
  } catch (err) {
