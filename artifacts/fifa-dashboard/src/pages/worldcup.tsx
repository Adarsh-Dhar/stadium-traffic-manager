...
export default function WorldCup() {
  ...
  const matchesQuery = useGetWorldCupMatches();
  const bracketQuery = useGetWorldCupBracket();

  const groups = standingsQuery.data?.standings || [];
  const matches = matchesQuery.data?.matches || [];
  const bracket = bracketQuery.data?.matches || [];
  const loading = standingsQuery.isLoading || matchesQuery.isLoading || bracketQuery.isLoading;
          ...
          {group.table?.map((team) => (
            <div key={team.team?.id} className="flex items-center justify-between p-2 rounded bg-secondary/20">
              <div className="flex items-center gap-2">
                ...
                {team.team?.crest && (
                  <img src={team.team.crest} alt={team.team.name ?? 'Unknown'} className="h-5 w-5 rounded-full" />
                )}
                <span className="text-sm font-medium">{team.team?.name ?? 'Unknown'}</span>
              </div>
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-3 flex-1">
        {match.homeTeam?.crest && (
          <img src={match.homeTeam.crest} alt={match.homeTeam.name ?? 'Unknown'} className="h-6 w-6 rounded-full" />
        )}
        <span className="text-sm font-semibold">{match.homeTeam?.name ?? 'Unknown'}</span>
      </div>
      ...
      <div className="flex items-center gap-3 flex-1 justify-end">
        <span className="text-sm font-semibold">{match.awayTeam?.name ?? 'Unknown'}</span>
        {match.awayTeam?.crest && (
          <img src={match.awayTeam.crest} alt={match.awayTeam.name ?? 'Unknown'} className="h-6 w-6 rounded-full" />
        )}
      </div>
    </div>
  );
